...
| 테스트          | Vitest + Testing Library                | 1,052개 · 커버리지 86%                                              |
...
| 개선                                                         | 결과                                                           |
| ------------------------------------------------------------ | -------------------------------------------------------------- |
| 아바타 업로드 리사이즈 상한 축소 (1600px→264px)              | 픽셀 면적 −97.3%                                               |
| 파비콘 전용 에셋 분리                                        | −92.5%(401KB→30KB)                                             |
| 채팅 리스트 가상화 (`@tanstack/react-virtual`)               | 1만 건 기준 스크립트 −53%(2.77s→1.28s), 렌더링 −80%(564→114ms) |
| 업로드 전 클라이언트 리사이즈 (4000px→1600px)                | 픽셀 면적 −84.0%                                               |
| Sentry SDK 지연 초기화 (동적 import + `requestIdleCallback`) | 초기 로딩 JS −45%(820→451KB)                                   |
| 지도 SDK 지연 로딩 (`next/dynamic`)                          | 홈 JS −18%(2,743→2,261KB), 전송량 −11%(524→468KB)              |
| 인증 왕복 축소: `getUser()` → `getClaims()`                  | 비대칭 서명 + JWKS 캐시 기준 왕복 2회→0회                      |
| Sentry 지연 초기화·파비콘 분리·아바타 리사이즈 축소 묶음     | 초기 로딩 1.63s → 1.40s (−14.1%)                               |
